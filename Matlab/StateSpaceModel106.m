function StateSpaceModelN = StateSpaceModel106(StateSpaceModelN)
    % 初始化结构体中的变量
    StateSpaceModelN.PortName = 'Tested Model 1 v0.00';
    StateSpaceModelN.PortIntroduction = 'NNSSE for UKF, simulate a recurrent neural network with activation function Tanh';

    StateSpaceModelN.Scale = 10000;
    ...
    if ~isfield(StateSpaceModelN, 'CurrentObservation')
        error('"CurrentObservation" needs to be set before initiate StateSpaceModel106');
    end
    ...
    % 定义结构体中的函数句柄
    StateSpaceModelN.StateTransitionEquation = @(In_State, StateSpaceModelN) StateSpaceModel106StateTransitionFunction(In_State, StateSpaceModelN);
    StateSpaceModelN.ObservationEquation = @(In_State, StateSpaceModelN) StateSpaceModel106ObservationFunction(In_State, StateSpaceModelN);
    StateSpaceModelN.PredictionEquation = @(In_State, StateSpaceModelN) StateSpaceModel106PredictionFunction(In_State, StateSpaceModelN);
    StateSpaceModelN.EstimatorPort = @(StateSpaceModelN) StateSpaceModel106EstimatorPort(StateSpaceModelN);
    StateSpaceModelN.EstimatorPortTermination = @() StateSpaceModel106EstimatorPortTermination();
end

% 定义各个函数的实现
function [Out_State, StateSpaceModelN] = StateSpaceModel106StateTransitionFunction(In_State, StateSpaceModelN)
    ...
    for i =1:StateSpaceModelN.MiddleLayer
        ...
        MiddleLayer(i) = (exp(MiddleLayer(i)) - exp(-MiddleLayer(i))) / (exp(MiddleLayer(i)) + exp(-MiddleLayer(i)));
...
function [Out_Observation, StateSpaceModelN] = StateSpaceModel106ObservationFunction(In_State, StateSpaceModelN)
    Out_Observation = In_State(1);
end

function [Out_PredictedState, StateSpaceModelN] = StateSpaceModel106PredictionFunction(In_State, StateSpaceModelN)
    InputLayer = In_State((StateSpaceModelN.PredictStep+1):StateSpaceModelN.NodeCount)/StateSpaceModelN.Scale ;
    Layer12Weight = In_State((StateSpaceModelN.NodeCount+1):(StateSpaceModelN.NodeCount+StateSpaceModelN.InputLayer * StateSpaceModelN.MiddleLayer));
    Layer23Weight = In_State((StateSpaceModelN.NodeCount+StateSpaceModelN.InputLayer * StateSpaceModelN.MiddleLayer+1):end);
    MiddleLayer = zeros(StateSpaceModelN.MiddleLayer,1);
...
function StateSpaceModelN = StateSpaceModel106EstimatorPort(StateSpaceModelN)
    StateSpaceModelN = Estimator3002(StateSpaceModelN);
end

function StateSpaceModel106EstimatorPortTermination()
