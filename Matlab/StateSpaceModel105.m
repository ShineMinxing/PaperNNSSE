function StateSpaceModelN = StateSpaceModel105(StateSpaceModelN)
    % 初始化结构体中的变量
    StateSpaceModelN.PortName = 'Tested Model 1 v0.00';
    StateSpaceModelN.PortIntroduction = 'NNSSE for UKF, simulate a recurrent neural network as 10-10-1';

    StateSpaceModelN.Scale = 1;
    StateSpaceModelN.PredictStep = 3;
    StateSpaceModelN.InputLayer = 10;
    ...
    if ~isfield(StateSpaceModelN, 'CurrentObservation')
        error('"CurrentObservation" needs to be set before initiate StateSpaceModel105');
    end
    ...
    % 定义结构体中的函数句柄
    StateSpaceModelN.StateTransitionEquation = @(In_State, StateSpaceModelN) StateSpaceModel105StateTransitionFunction(In_State, StateSpaceModelN);
    StateSpaceModelN.ObservationEquation = @(In_State, StateSpaceModelN) StateSpaceModel105ObservationFunction(In_State, StateSpaceModelN);
    StateSpaceModelN.PredictionEquation = @(In_State, StateSpaceModelN) StateSpaceModel105PredictionFunction(In_State, StateSpaceModelN);
    StateSpaceModelN.EstimatorPort = @(StateSpaceModelN) StateSpaceModel105EstimatorPort(StateSpaceModelN);
    StateSpaceModelN.EstimatorPortTermination = @() StateSpaceModel105EstimatorPortTermination();
end

% 定义各个函数的实现
function [Out_State, StateSpaceModelN] = StateSpaceModel105StateTransitionFunction(In_State, StateSpaceModelN)
...
function [Out_Observation, StateSpaceModelN] = StateSpaceModel105ObservationFunction(In_State, StateSpaceModelN)
    Out_Observation = In_State(1);
end

function [Out_PredictedState, StateSpaceModelN] = StateSpaceModel105PredictionFunction(In_State, StateSpaceModelN)
    InputLayer = In_State((StateSpaceModelN.PredictStep+1):StateSpaceModelN.NodeCount)/StateSpaceModelN.Scale ;
    Layer12Weight = In_State((StateSpaceModelN.NodeCount+1):(StateSpaceModelN.NodeCount+StateSpaceModelN.InputLayer * StateSpaceModelN.MiddleLayer));
    Layer23Weight = In_State((StateSpaceModelN.NodeCount+StateSpaceModelN.InputLayer * StateSpaceModelN.MiddleLayer+1):end);
    MiddleLayer = zeros(StateSpaceModelN.MiddleLayer,1);
...
function StateSpaceModelN = StateSpaceModel105EstimatorPort(StateSpaceModelN)
    StateSpaceModelN = Estimator3002(StateSpaceModelN);
end

function StateSpaceModel105EstimatorPortTermination()
