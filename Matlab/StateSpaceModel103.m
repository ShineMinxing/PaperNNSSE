function StateSpaceModelN = StateSpaceModel103(StateSpaceModelN)
    ...
    StateSpaceModelN.Matrix_Q = diag([ones(1,StateSpaceModelN.Np + StateSpaceModelN.PredictStep - 1), 0.00001*ones(1, StateSpaceModelN.Np)]);
...
function [Out_State, StateSpaceModelN] = StateSpaceModel103StateTransitionDiffFunction(In_State, StateSpaceModelN)
    ...
    idx_a = StateSpaceModelN.PredictStep:(StateSpaceModelN.PredictStep + StateSpaceModelN.Np - 1);  % 第一段索引
    idx_b = (StateSpaceModelN.PredictStep + StateSpaceModelN.Np):StateSpaceModelN.Nx;             % 第二段索引

    Out_State(1 , idx_a) = In_State(idx_b);
    Out_State(1 , idx_b) = In_State(idx_a);

     for i = 2:(StateSpaceModelN.PredictStep  + StateSpaceModelN.Np - 1)
        Out_State(i, i - 1) = 1;
    end

    for i = (StateSpaceModelN.PredictStep  + StateSpaceModelN.Np):StateSpaceModelN.Nx
        Out_State(i, i) = 1;
    end
end

function [Out_Observation, StateSpaceModelN] = StateSpaceModel103ObservationFunction(In_State, StateSpaceModelN)
    Out_Observation = In_State((StateSpaceModelN.PredictStep):(StateSpaceModelN.PredictStep + StateSpaceModelN.Np-1))' * In_State((StateSpaceModelN.PredictStep + StateSpaceModelN.Np) : end);
end
