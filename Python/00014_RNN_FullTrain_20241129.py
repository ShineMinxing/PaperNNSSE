...
# 将保存路径设置为脚本文件所在的同一目录
model_save_path = os.path.join(script_dir, f'{train_base_name}_RNN.pth')
prediction_save_path = os.path.join(script_dir, f'{train_base_name}_RNN.csv')
scaler_save_path_X = os.path.join(script_dir, f'{train_base_name}_RNN_scaler_X.gz')
scaler_save_path_y = os.path.join(script_dir, f'{train_base_name}_RNN_scaler_y.gz')
...
X_train = torch.tensor(X_train, dtype=torch.float32).unsqueeze(2).to(device)  # (samples, seq_len, 1)
...
test_targets = test_df.iloc[:, targets_column].values      # 第二列：2 步后的真实数据
