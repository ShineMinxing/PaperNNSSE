...
prediction_save_path = os.path.join(script_dir, f'{train_base_name}_Transformer.csv')
...
# Transformer 的输入需要形状为 (seq_len, batch_size, feature_size)
X_train = torch.tensor(X_train, dtype=torch.float32).unsqueeze(2).permute(1, 0, 2).to(device)  # (seq_len, batch_size, feature_size)
y_train = torch.tensor(y_train, dtype=torch.float32).unsqueeze(1).to(device)  # (batch_size, 1)
...
class TransformerModel(nn.Module):
    def __init__(self, input_size=1, d_model=64, hidden_size=128, num_layers=2, nhead=8, output_size=1, dropout=0.1):
        ...
        self.input_fc = nn.Linear(input_size, d_model)  # 新增：将输入映射到 d_model 维度
    ...
    def forward(self, src):
        ...
        src = self.input_fc(src)  # 映射到 d_model 维度
...
# 转换为 PyTorch 张量并移动到设备
X_test = torch.tensor(X_test, dtype=torch.float32).unsqueeze(2).permute(1, 0, 2).to(device)  # (seq_len, batch_size, feature_size)
y_test = torch.tensor(y_test, dtype=torch.float32).unsqueeze(1).to(device)  # (batch_size, 1)
...
for epoch in range(num_epochs):
    ...
    for X_batch, y_batch in train_loader:
        X_batch = X_batch.permute(1, 0, 2).to(device)  # (seq_len, batch_size, feature_size)
    ...
    with torch.no_grad():
        ...
        test_loss = criterion(outputs.squeeze(), y_test.squeeze()).item()
